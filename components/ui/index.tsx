...
export function SectionHeader({ eyebrow, title, subtitle, centered = true, light = false }: { eyebrow?: string; title: string; subtitle?: string; centered?: boolean; light?: boolean }) {
  return (
    <div className={`mb-12 ${centered ? "text-center" : ""}`}>
      ...
      <h2 className={`font-display text-3xl md:text-4xl font-bold leading-tight ${light ? "text-white" : "text-navy-dark"}`}>{title}</h2>
      {subtitle && (
        <p className={`mt-4 text-base md:text-lg leading-relaxed max-w-2xl ${centered ? "mx-auto" : ""} ${light ? "text-white/65" : "text-slate"}`}>{subtitle}</p>
      )}
    </div>
  );
}
...
export function PageHero({ title, subtitle, breadcrumb }: { title: string; subtitle?: string; breadcrumb?: { label: string; href: string }[] }) {
  return (
    <div className="bg-navy-dark text-white py-14 md:py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        {breadcrumb && (
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-white/35 mb-4">
        ...
        {subtitle && <p className="mt-3 text-white/55 text-lg max-w-2xl leading-relaxed">{subtitle}</p>}
      </div>
    </div>
  );
}
